...
function UpgradeGroup:CanApplySubGroupToTarget(target, subGroup)
	if subGroup.ModRequirement ~= nil and not Ext.Mod.IsModLoaded(subGroup.ModRequirement) then
		return false
	end
...
function UpgradeGroup:Apply(target, applyImmediately, hardmodeOnly)
	if self.DisabledFlag == "" or GlobalGetFlag(self.DisabledFlag) == 0 then
		...
		if roll > 0 then
				...
				if roll >= v.StartRange and roll <= v.EndRange and self:CanApplySubGroupToTarget(target, v) then
					if v.ID ~= "None" then
						if v:Apply(target, applyImmediately, hardmodeOnly) then
							--Ext.Utils.Print(string.format("[EUO] Roll success (%i/%i)! Group(%s:%s) Range(%i-%i)", roll, Vars.UPGRADE_MAX_ROLL, self.ID, v.ID, v.StartRange, v.EndRange))
			...
			if successes > 0 then
				--Ext.Utils.Print(string.format("[EUO] Applied (%i) subgroups for group(%s) to target(%s)", successes, self.ID, target))
			end
			return successes > 0
		else
